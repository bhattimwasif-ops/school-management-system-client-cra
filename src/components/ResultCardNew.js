...
const ResultCardNew = ({ selectedStudentId, resultData }) => {
  const [localResultData, setLocalResultData] = useState(resultData || null);
  ...
  useEffect(() => {
    const fetchResultData = async () => {
      if (resultData) {
        // Use provided resultData if available, no fetch needed
        setLocalResultData(resultData);
        setError('');
        return;
      }
      if (!selectedStudentId) {
        setLocalResultData(null);
      ...
      try {
        ...
        const response = await axios.get(`${BASE_URL}/api/studenttest/${selectedStudentId}/result?testId={testIdPlaceholder}`, {
          headers: { Authorization: `Bearer ${token}` },
        });
        console.log('API Response:', response.data);
        setLocalResultData(response.data);
        setError('');
      } catch (err) {
    ...
    fetchResultData();
  }, [selectedStudentId, resultData]); // Depend on both props

  if (error) return <div style={styles.container}><p style={{ color: 'red' }}>{error}</p></div>;
  if (!localResultData) return <div style={styles.container}><p>Loading...</p></div>;
  ...
  return (
    <div style={styles.container}>
      ...
      <h2 style={styles.subHeader}>Group: {localResultData.group || 'Science'}</h2>

      <table style={styles.infoTable}>
        <tbody>
          <tr>
            <td style={styles.infoCell}><strong>Name:</strong></td><td style={styles.infoCell}>{localResultData.name || 'N/A'}</td>
            <td style={styles.infoCell}><strong>Roll No:</strong></td><td style={styles.infoCell}>{localResultData.rollNo || 'N/A'}</td>
          </tr>
          <tr>
            <td style={styles.infoCell}><strong>Father's Name:</strong></td><td style={styles.infoCell}>{localResultData.fatherName || 'N/A'}</td>
            <td style={styles.infoCell}><strong>Father's CNIC:</strong></td><td style={styles.infoCell}>{localResultData.fatherCNIC || 'N/A'}</td>
          </tr>
          <tr>
            <td style={styles.infoCell}><strong>Date of Birth:</strong></td><td style={styles.infoCell}>{localResultData.dateOfBirth || 'N/A'}</td>
            <td style={styles.infoCell}><strong>Institution / District:</strong></td>
            <td style={styles.infoCell}>{localResultData.institution || 'N/A'}</td>
          </tr>
        </tbody>
      </table>
      ...
      <table style={styles.table}>
        ...
        <tbody>
          {(localResultData.subjects || []).map((subjectData, index) => (
            <tr key={index}>
          ...
          <tr>
            ...
            <th style={styles.th}>{localResultData.totalStatus} {localResultData.totalMarks || 'N/A'}</th>
          </tr>
        </tbody>
      </table>
      ...
      <div style={styles.footer}>
        <div><strong>Result Declared On:</strong><br />{localResultData.resultDeclaredOn || '20 Aug 2025'}</div>
